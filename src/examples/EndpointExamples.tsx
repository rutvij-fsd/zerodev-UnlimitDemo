...
const EndpointExamples: React.FC = () => {
    ...
    const handleCustomOrderIdChange = (e: { target: { value: SetStateAction<string>; }; }) => {
        setCustomOrderId(e.target.value);
    };
    
    // Event handler for wallet address field
    const handleWalletAddressChange = (e: { target: { value: SetStateAction<string>; }; }) => {
    ...
    const getConfig = async () => {
        ...
        const response = await fetch(`http://localhost:3000/api/proxy?endpoint=/onramp/v1/configuration`, {
            method: "GET",
            headers: {
    ...
    const getOrders = async (params: string) => {
        const response = await fetch(`http://localhost:3000/api/proxy?endpoint=/onramp/v1/orders&${params}`, {
            method: "GET",
            redirect: 'follow',
            headers: {
                "access-control-allow-headers": "Accept",
    ...
    const getSingleOrder = async (e: { preventDefault: () => void; }) => {
        ...
        const response = await fetch(`http://localhost:3000/api/proxy?endpoint=/onramp/v1/orders/${customOrderId}&walletAddress=${walletAddress}`, {
            method: "GET",
            redirect: 'follow',
            headers: {
                "access-control-allow-headers": "Accept",
...
const getQuotes = async (): Promise<QuoteData | null> => {
  // Build the URL query string from the form values
    const queryString = new URLSearchParams(form).toString();
    const response = await fetch(`http://localhost:3000/api/proxy?endpoint=/onramp/v1/quotes&${queryString}`, {
        method: "GET",
        redirect: 'follow',
        headers: {
            "access-control-allow-headers": "Accept",
